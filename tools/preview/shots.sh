...
for flavor in $flavors; do
	...
	for variant in $variants; do
		...
		# A tall screen, explicitly. The gallery self-captures its window's full
		# allocation, but Xvfb's default screen is ~1024px high and nothing
		# below that is ever painted — so as sections were added, the bottom of
		# the sheet silently became black. The Feedback section (level bars,
		# spinner) sat in that band, which is where a mid-range level bar drew
		# no fill for two phases without any sheet showing it.
		xvfb-run -a --server-args="-screen 0 1000x2200x24" \
			env GTK_THEME="$theme" /usr/bin/python3 "$here/gallery.py" \

...
- **Whisker two-tone**: the popup-menu surface must cover every pane of a popup, or none. Phase 2 resolved the reported `#171717`/`#404040` split in `menu.mjs` — **against the wrong widget tree.** Its gallery modelled Whisker as two `GtkListBox`es in a `.menu` box; the real plugin is a `GtkTreeView` of apps beside a column of `GtkToggleButton.category-button`, inside a `GtkWindow` whose widget *name* is `whiskermenu-window`. The post-phase-5 spot-check measured the live plugin still two-tone one step down — tree view on `bg_sunk`, button column on `bg`, plain on Noon. Fixed in `0adccd2`: `#whiskermenu-window` takes the popup surface and its tree view goes transparent, measured one colour at every sample point on both flavours, and the gallery now instantiates the real widgets. The same check found a checked `.flat` button drawing `accent_on` text on no fill (source order let `.flat` strip `:checked`'s background) — Whisker's active category, and Thunar's current-location pathbar button — fixed on both GTK3 and GTK4.
...
- **A gallery can instantiate the wrong widgets and verify those instead.** Phase 2's Whisker section built two list boxes; the plugin builds a tree view and a column of toggle buttons. Every sheet since then showed a fixed Whisker while the real one stayed two-tone one step down. When a section claims to reproduce a specific app, its widget tree has to be checked against that app — by `strings` on the binary for names and classes, and by measurement on the live window — not assumed from what the app looks like.

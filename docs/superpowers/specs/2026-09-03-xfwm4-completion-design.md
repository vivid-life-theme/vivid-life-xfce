...
| Titlebar bottom border | ~~1px `border.default`~~ — **superseded**, see Frame specification below | `.ks-bar { border-bottom: 1px solid var(--vl-border) }`. The Frame specification reassigns this region to the 2px focus edge, and the code follows that: the bottom 2px of every titlebar asset is the accent (active) or `border.subtle` (inactive). Do not re-add a `border.default` row — it would either eat 1px of the focus edge or push the bar to 33px. |
